...
docker exec $CONTAINER_NAME bash -c "echo 'IT WORKS STATIC IN NGINX PHP-FPM' > /usr/src/app/public/static.txt"
docker exec --user root:root $CONTAINER_NAME apk add -U nginx > /dev/null
docker exec --user root:root $CONTAINER_NAME bash -c "echo 'server { listen 8080 default_server; root /usr/src/app/public;     location / { try_files \$uri /index.php\$is_args\$args; }
...
docker exec --user root:root -d $CONTAINER_NAME nginx -g 'daemon off;'
docker exec $CONTAINER_NAME bash -c 'wget localhost:8080/index.php -q -O /tmp/response && cat /tmp/response' | grep "IT WORKS IN NGINX PHP-FPM" > /dev/null
...
docker exec $CONTAINER_NAME bash -c 'wget localhost:8080/static.txt -q -O /tmp/response && cat /tmp/response' | grep "IT WORKS STATIC IN NGINX PHP-FPM" > /dev/null
