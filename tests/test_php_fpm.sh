...
docker exec $CONTAINER_NAME apk add -U nginx
docker exec $CONTAINER_NAME bash -c "echo 'server { listen 8080 default_server; root /usr/src/app/public; location ~ \.php$ {fastcgi_pass 127.0.0.1:9000; fastcgi_split_path_info ^(.+\.php)(/.*)$; include fastcgi_params; fastcgi_param SCRIPT_FILENAME \$realpath_root\$fastcgi_script_name; fastcgi_param DOCUMENT_ROOT \$realpath_root;}}' > /etc/nginx/http.d/default.conf"
docker exec -d $CONTAINER_NAME php-fpm81 --nodaemonize
